...
def validate_objects(objects: List[Dict[str, Any]]) -> None:
    ...
    for i, obj in enumerate(objects):
        if not isinstance(obj, dict):
            raise RedisPublishError(
                f"Object at index {i} must be a dict, got {type(obj).__name__}"
            )
        ...
        if "class_name" not in obj:
            raise RedisPublishError(
                f"Object at index {i} missing required field 'class_name'"
            )
...
def validate_image(image: np.ndarray) -> None:
    ...
    if image.ndim == 3 and image.shape[2] not in (1, 3, 4):
        raise InvalidImageError(
            f"Image channels must be 1, 3, or 4, got {image.shape[2]}"
        )
    ...
    if image.shape[0] > max_dimension or image.shape[1] > max_dimension:
        raise InvalidImageError(
            f"Image dimensions too large: {image.shape}. "
...
def validate_stream_name(stream_name: str) -> None:
    ...
    if not isinstance(stream_name, str):
        raise ValueError(
            f"Stream name must be a string, got {type(stream_name).__name__}"
        )
    ...
    if len(stream_name) > 255:
        raise ValueError(
            f"Stream name too long ({len(stream_name)} chars). Maximum is 255"
        )
    ...
    for char in invalid_chars:
        if char in stream_name:
            raise ValueError(
                f"Stream name contains invalid character: {repr(char)}"
            )
